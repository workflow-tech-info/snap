...
export function AdminSidebar({ activeSection, onSectionChange, isCollapsed, onToggleCollapse }: AdminSidebarProps) {
  ...
  const menuItems = [
    ...
    { id: 'installations', label: 'Installations', icon: CheckCircle2 },
    { id: 'devices', label: 'Devices', icon: Cpu },
    { id: 'vehicles', label: 'Vehicles', icon: Car },
    { id: 'workshops', label: 'Workshops', icon: Warehouse },
    { id: 'inventory', label: 'Inventory', icon: Package },
    { id: 'technicians', label: 'Technicians', icon: Users },
    { id: 'service', label: 'Service Jobs', icon: Wrench },
          ...
          <Link to="/" className="flex items-center gap-3 hover:opacity-80 transition-opacity">
            ...
            {!isCollapsed && (
              <div className="flex flex-col animate-in fade-in duration-500">
                <span className="text-xl font-bold tracking-tighter text-white uppercase italic">INSTALL HUB</span>
                <span className="text-[10px] font-black text-primary uppercase tracking-widest leading-none">Ops Command</span>
              </div>
            )}
          </Link>
        </div>
